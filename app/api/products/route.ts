...
function getStockHealth(onHand: number): StockHealth {
...
export async function GET() {
  ...
  try {
    ...
    const products: ProductRow[] = Array.from(bySku.entries()).map(([sku, item], idx) => {
      ...
      const stockHealth = getStockHealth(item.onHand);
