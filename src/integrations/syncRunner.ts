...
import { readJsonFile, writeJsonFile, withLock, getConnections } from "./storage";
...
export async function syncMarketplace(
  ...
  try {
    ...
    // Real API sync
    let marketplaceCreds: Record<string, string>;
    if (typeof connection.creds === "string") {
      marketplaceCreds = decryptCredentials(connection.creds);
    } else {
      marketplaceCreds = connection.creds as Record<string, string>;
    }
